...
async function main() {
  ...
  try {
    ...
    var urlRegexV22 = new RegExp(
      "^https?:\\/\\/.*\\/playback\\/presentation\\/2\\.0\\/" +
        playbackFile +
        "\\?meetingId=[a-z0-9]{40}-[0-9]{13}"
    );
    var urlRegexV23 = new RegExp(
      "^https?:\\/\\/.*\\/playback\\/presentation\\/2\\.3\\/[a-z0-9]{40}-[0-9]{13}"
    );
    ...
    if (!exportname || exportname == "MEETING_ID") {
      exportname = bbbVersionIs23
        ? url.split("2.3/")[1] + ".webm"
        : url.split("=")[1] + ".webm";
    }
    ...
    if (!bbbVersionIs23) {
      ...
      await page.click('button[class~="vjs-play-control"]', {
        waitUntil: "domcontentloaded",
      });
    }
